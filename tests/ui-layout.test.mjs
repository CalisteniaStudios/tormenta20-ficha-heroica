...
assert.match(script, /name="layout"/, "the logo dialog includes a tabbed or classic layout choice");
...
assert.match(script, /classicLayout: appearance\.layout === "classic"/, "the classic template is selected from the personal preference");
assert.doesNotMatch(script, /sections\.addClass\("active"\)/, "the layout keeps normal Foundry tab behavior");
...
assert.match(characterSheet, /class="sheet-tabs tabs primary flexrow t20ga-classic-nav"/, "classic mode has its compact navigation");
assert.equal((characterSheet.match(/class="item" data-tab="(?:attributes|journal|effects)"/g) ?? []).length, 3, "classic navigation exposes Sheet, Journal, and Effects");
assert.match(characterSheet, /class="tab attributes t20ga-classic-page"/, "classic mode gathers character information on the Sheet tab");
assert.match(characterSheet, /t20ga\.list-weapon[\s\S]*t20ga\.list-equipment[\s\S]*t20ga\.list-powers-tabbed[\s\S]*t20ga\.list-spells/, "the classic left column follows the original sheet organization");
assert.match(characterSheet, /t20ga\.list-skills[\s\S]*t20ga\.list-consumable[\s\S]*t20ga\.list-loot/, "the classic right column keeps skills and general inventory together");
...
assert.match(campaignConfig, /name="logoPushPortrait"/, "the GM can optionally reserve portrait space for an enlarged logo");
assert.match(campaignConfig, /name="portraitWidth" min="20" max="45"/, "the GM can adjust the portrait panel width");
...
assert.match(characterSheet, /--t20ga-hero-panel-width: \{\{t20ga\.portraitWidth\}\}%/, "the explicit portrait width reaches the character sheet");
assert.match(css, /grid-template-columns:\s*minmax\(245px, var\(--t20ga-hero-panel-width, 25%\)\) minmax\(0, 1fr\);/, "the portrait column follows only its dedicated width control");
...
assert.match(css, /\.t20ga-campaign-logo-frame\s*\{[^}]*height:\s*var\(--t20ga-campaign-logo-reserved-height, 64px\);[^}]*overflow:\s*visible;/s, "the optional reserved height can push the portrait below an enlarged logo");
...
assert.match(script, /function previewCampaignLogoLayout\(\{ scale, x, y, logoPushPortrait, portraitWidth \}\)/, "campaign logo and portrait adjustments can be applied to open sheets");
...
assert.match(script, /shell\?\.style\.setProperty\("--t20ga-hero-panel-width", `\$\{normalizedPortraitWidth\}%`\)/, "the live preview updates the open sheet portrait width");
assert.match(script, /frame\.style\.setProperty\("--t20ga-campaign-logo-reserved-height", `\$\{reservedHeight\}px`\)/, "the live preview optionally moves the portrait below the logo");
assert.match(script, /previewCampaignLogoLayout\(layout\)/, "moving a layout control updates the open sheets immediately");
